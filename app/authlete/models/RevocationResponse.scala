...
object RevocationResponse {
  ...
  given jsonCodec: JsonValueCodec[RevocationResponse] = JsonCodecMaker.make(codecMakerConfig)

  given encoderRevocationResponse: Encoder[RevocationResponse] = Encoder.instance { t =>
    Json.fromFields {
      Seq(
  ...
  given decoderRevocationResponse: Decoder[RevocationResponse] = Decoder.instance { c =>
    for {
      resultCode      <- c.downField("resultCode").as[Option[String]]
      resultMessage   <- c.downField("resultMessage").as[Option[String]]
      action          <- mapEmptyStringToNull(c.downField("action")).as[Option[RevocationResponseAction]]
