...
/**
  ...
  *   application. Its format varies depending on the value of `action` parameter. See description
  *   for details.
  */
...
object TokenFailResponse {
  ...
  given jsonCodec: JsonValueCodec[TokenFailResponse] = JsonCodecMaker.make(codecMakerConfig)

  given encoderTokenFailResponse: Encoder[TokenFailResponse] = Encoder.instance { t =>
    Json.fromFields {
      Seq(
  ...
  given decoderTokenFailResponse: Decoder[TokenFailResponse] = Decoder.instance { c =>
    for {
      resultCode      <- c.downField("resultCode").as[Option[String]]
      resultMessage   <- c.downField("resultMessage").as[Option[String]]
      action          <- mapEmptyStringToNull(c.downField("action")).as[Option[TokenFailResponseAction]]
