...
/**
  ...
  *   application. The value of `parameters` is the entire entity body (which is formatted in
  ...
  *   client application. If the device authorization endpoint of the authorization server
  *   implementation supports Basic `Authentication` as a means of client authentication, and the
  *   request from the client application contained its client ID in `Authorization` header, the
  *   value should be extracted and set to this parameter.
  * @param clientSecret
  *   The client secret extracted from `Authorization` header of the device authorization request
  *   from the client application. If the device authorization endpoint of the authorization server
  *   implementation supports Basic Authentication as a means of client authentication, and the
  *   request from the client application contained its client secret in `Authorization` header, the
  *   value should be extracted and set to this parameter.
  * @param clientCertificate
  *   The client certificate used in the TLS connection between the client application and the
  *   device authorization endpoint of the authorization server.
